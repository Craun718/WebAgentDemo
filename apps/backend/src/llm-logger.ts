...
function logToConsole(entry: LlmLogEntry) {
  if (entry.direction === "request") {
    ...
    const usageStr = entry.usage ? ` | tokens=${entry.usage.total_tokens}` : "";
    const reasonStr = entry.finishReason ? ` | reason=${entry.finishReason}` : "";
    const toolStr = entry.toolCalls?.length
      ? ` | tools=${entry.toolCalls.map((t) => `${t.function.name}(${truncate(t.function.arguments.replace(/\n/g, " "), 40)})`).join(", ")}`
      : "";
    console.log(
      `[LLM←RES] #${entry.requestId} | ${entry.durationMs}ms${errorStr}${usageStr}${reasonStr}${toolStr} | "${snippet}"`,
    );
  }
}
...
export function logRequest(entry: Omit<LlmRequestLog, "timestamp" | "direction">) {
...
export function logResponse(entry: Omit<LlmResponseLog, "timestamp" | "direction">) {
