...
/** Sample tool: returns the current time, optionally in a given IANA timezone. */
const getCurrentTime: AgentTool = {
  name: "get_current_time",
  description:
    "Get the current date and time in the user's locale. " +
    "Pass `timeZone` as an IANA identifier (e.g. 'Asia/Shanghai', 'America/New_York') to render the time in that zone; omit it to use the browser's default.",
  ...
  parameters: Type.Object({
    // Optional IANA timezone identifier. When omitted, the browser default is used.
  ...
  execute: ({ timeZone }) => {
    if (timeZone === undefined) return new Date().toLocaleString();
